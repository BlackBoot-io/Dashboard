...
import { useSignupMutation } from "api/account";
...
const Signup = () => {
  ...
  const invalid = () => {};
  return (
    <div id="signup" className="form-wrapper">
      ...
      >
        <Space direction="vertical" size={5} className="w-100">
          ...
          <Form.Item>
            <button type="button" className="btn-secondary google-auth">
              <svg
                width="17"
                height="18"
                viewBox="0 0 17 18"
      ...
      <div className="redirect">
        ...
        <Link to={`/${routes.auth}/${routes.login}`}>{t("signin")}</Link>
      </div>
    </div>
  );
};
